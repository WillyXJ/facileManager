...
class fm_module_settings {
	...
	function save() {
		...
		foreach ($_POST as $key => $data) {
			if (!in_array($key, $exclude)) {
				...
				$current_value = getOption($key, $_SESSION['user']['account_id'], 'fm_' . $__FM_CONFIG['fmSQLPass']['prefix'] . 'options');
				if (is_array($current_value)) $current_value = implode("\n", $current_value);
				if ($current_value == $data) continue;
				
				/** Ensure we are not deleting a region that servers are associated with */
				if ($key == 'regions') {
					$submitted_regions = explode("\n", str_replace("\r\n", "\n", $data));
					$current_value = explode("\n", $current_value);
					foreach ($current_value as $sub_region) {
						$sub_region = trim($sub_region);
						if (!in_array($sub_region, $submitted_regions) && $sub_region) {
							basicGet('fm_' . $__FM_CONFIG['fmSQLPass']['prefix'] . 'servers', $sub_region, 'server_', 'server_region');
							if ($fmdb->num_rows) return 'There are still servers associated with the ' . $sub_region . ' region.';
						}
					}
				}
		...
		if (is_array($new_array)) {
			foreach ($new_array as $option => $value) {
				...
				if (is_array($__FM_CONFIG['fmSQLPass']['default']['options'][$option]['default_value'])) {
					$temp_array = explode("\n", $option_value);
					$temp_value = array();
				...
				$result = setOption($option, $option_value, $command, $_SESSION['user']['account_id'], 'fm_' . $__FM_CONFIG['fmSQLPass']['prefix'] . 'options');
	 ...
	 */
	function printForm() {
		global $fmdb, $__FM_CONFIG, $allowed_to_manage_backup_options;
		
		$disabled = $allowed_to_manage_backup_options ? null : 'disabled';
		
		$save_button = $disabled ? null : '<input type="submit" name="save" id="save_module_settings" value="Save" class="button" />';
		
		$query = "SELECT * FROM fm_{$__FM_CONFIG['fmSQLPass']['prefix']}options WHERE account_id={$_SESSION['user']['account_id']}";
		...
		$option_rows = buildSettingsForm($saved_options, $__FM_CONFIG['fmSQLPass']['default']['options']);
