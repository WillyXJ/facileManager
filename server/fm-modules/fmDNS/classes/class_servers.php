...
//		if (currentUserCan('manage_servers', $_SESSION['module'])) $bulk_actions_list = array('Enable', 'Disable', 'Delete', 'Upgrade');
		...
		if (!$result) {
			printf('<p id="table_edits" class="noresult" name="servers">%s</p>', _('There are no servers.'));
		} else {
			...
			if ($fmdb->num_rows) {
				...
				for ($i=0; $i < $count; $i++) {
					...
					if (!$fmdb->rows_affected) {
						return _('The associated zones for this server could not be updated because a database error occurred.');
					}
				}
			}
...
HTML;
	...
	function printForm($data = '', $action = 'add') {
		...
		$alternative_help = ($action == 'add') ? sprintf('<p>%s</p>', _('<b>Note:</b> The client installer can automatically generate this entry.')) : null;
